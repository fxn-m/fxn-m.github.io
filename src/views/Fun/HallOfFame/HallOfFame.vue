    ...
    <div class="game-log">
      <div>
        <div class="log-header">
          <h3>
            Attempts: {{ Object.values(scores).reduce((a, b) => a + b, 0) }}
          </h3>
...
function startNewGame() {
  ...
  while (
    cards.value[2].value === cards.value[1].value ||
    cards.value[2].value === cards.value[0].value
  ) {
...
function handleColorGuess(guess: Color) {
  ...
  gameLogs.value.push(
    `Stage 1: You guessed ${guess}... ${result ? correctHTML : incorrectHTML}`
  )
  if (result) {
    currentStage.value++
  } else {
...
function handleAboveBelowGuess(guess: "ABOVE" | "BELOW") {
  const result =
    (guess === "ABOVE" && cards.value[1].value > cards.value[0].value) ||
    (guess === "BELOW" && cards.value[1].value < cards.value[0].value)
  gameLogs.value.push(
    `Stage 2: You guessed ${guess}... ${result ? correctHTML : incorrectHTML}`
  )
  if (result) {
    currentStage.value++
  } else {
...
function handleBetweenOutsideGuess(guess: "BETWEEN" | "OUTSIDE") {
  const [min, max] = [
    Math.min(cards.value[0].value, cards.value[1].value),
    Math.max(cards.value[0].value, cards.value[1].value),
  ]
  const isBetween = cards.value[2].value > min && cards.value[2].value < max
  const result =
    (guess === "BETWEEN" && isBetween) || (guess === "OUTSIDE" && !isBetween)
  gameLogs.value.push(
    `Stage 3: You guessed ${guess}... ${result ? correctHTML : incorrectHTML}`
  )
  if (result) {
    currentStage.value++
  } else {
...
function makeFinalGuess() {
  if (finalGuessValue.value === null || finalGuessSuit.value === null) {
    gameLogs.value.push(
      "Please select both a value and a suit for your final guess."
    )
    return
  }
  const result =
