...
const resolveLinksDataSourceId = async (config: AppConfig, dataSourceId?: string) => {
...
const extractCategoriesFromDataSource = async (config: AppConfig, dataSourceId: string) => {
  const notion = createNotionClient(config.notionLinksSecret);
  ...
  if (!categories || categories.length === 0) {
    throw new Error(`Unable to resolve Categories property for Links source ${resolvedId}`);
  }
...
const getLinksMissingSummary = async (
  ...
  while (hasNextPage) {
    ...
    results = [...results, ...response.results.filter(hasId).map((item) => ({ id: item.id }))];
...
export const enrichLinkItem = async (config: AppConfig, pageId: string, dataSourceId: string) => {
  ...
  const resolvedDataSourceId = await resolveLinksDataSourceId(config, dataSourceId);
  const categories = await extractCategoriesFromDataSource(config, resolvedDataSourceId);
