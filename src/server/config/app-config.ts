...
const firstDefined = (source: ConfigSource, keys: string[]): string | undefined => {
...
const requireValue = (source: ConfigSource, label: string, keys: string[]): string => {
...
export const createAppConfig = (source: ConfigSource): AppConfig => ({
  ...
  spotifyClientId: requireValue(source, "SPOTIFY_CLIENT_ID", ["SPOTIFY_CLIENT_ID"]),
  spotifyClientSecret: requireValue(source, "SPOTIFY_CLIENT_SECRET", ["SPOTIFY_CLIENT_SECRET"]),
  spotifyRefreshToken: requireValue(source, "SPOTIFY_REFRESH_TOKEN", ["SPOTIFY_REFRESH_TOKEN"]),
  notionTabOverflowSecret: requireValue(source, "NOTION_TAB_OVERFLOW_SECRET", [
    "NOTION_TAB_OVERFLOW_SECRET",
  ]),
  notionTabOverflowDataSourceId: requireValue(source, "NOTION_TAB_OVERFLOW_DATA_SOURCE_ID", [
    "NOTION_TAB_OVERFLOW_DATA_SOURCE_ID",
  ]),
  notionLinksSecret: requireValue(source, "NOTION_LINKS_SECRET", ["NOTION_LINKS_SECRET"]),
  notionLinksDataSourceId: requireValue(source, "NOTION_LINKS_DATA_SOURCE_ID", [
    "NOTION_LINKS_DATA_SOURCE_ID",
  ]),
  notionBlogSecret: requireValue(source, "NOTION_BLOG_SECRET", ["NOTION_BLOG_SECRET"]),
  notionBlogDataSourceId: requireValue(source, "NOTION_BLOG_DATA_SOURCE_ID", [
    "NOTION_BLOG_DATA_SOURCE_ID",
  ]),
  stravaClientSecret: requireValue(source, "STRAVA_CLIENT_SECRET", ["STRAVA_CLIENT_SECRET"]),
  stravaRefreshToken: requireValue(source, "STRAVA_REFRESH_TOKEN", ["STRAVA_REFRESH_TOKEN"]),
  googleGenerativeAiApiKey: requireValue(source, "GOOGLE_GENERATIVE_AI_API_KEY", [
